...
"""
Xextr = ((1, 1), (0, 2), (-5, 9), (-6, 10))
Yextr = (200 + sum([i[0] for i in Xextr]), 200 + sum([i[1] for i in Xextr]))
m = 3
N = 15
Y = list(list(random.randrange(Yextr[0], Yextr[1]) for _ in range(m)) for _ in range(N))
p = 0.95
x0123n = (
    (1, -1, -1, -1),
    (1, -1, -1, 1),
...
while True:
    ...
    S = [sum([(Yavg[i] - Y[j][i]) ** 2 for i in range(m)]) / m for j in range(N)]
    Gp = max(S) / sum(S)
    F1 = m - 1
    F2 = N
    q = 1 - p
    ...
    if Gp < Gt:
        ...
        for i in range(len(Y)):
            m += 1
            Y[i].append(random.randrange(Yextr[0], Yextr[1]))
level = 2
S2_B = sum(Yavg) / N
while True:
